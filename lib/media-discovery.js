...
const LANGUAGE_PATTERNS = [
  ...
  { code: "pl", pattern: /\b(?:polish|polski|polska|lektor|dub(?:bed)?\s+polish|polish\s+dub)\b/i },
...
export function explainArchiveCandidate(candidate = {}, block = {}) {
  const quality = candidate.quality || archiveQualitySignals(candidate);
  const safetyBlocked = isClearlyPornographicArchiveCandidate(candidate);
  const language = archiveLanguageFit(block, candidate);
  const duration = Number(candidate.duration || 0);
  const minDuration = Number(block.minDuration || 45);
  const maxYear = Number(block.maxYear);
  const tooShort = duration > 0 && duration < minDuration;
  const tooNew = Number.isFinite(maxYear) && candidateLooksNewerThanBlock(candidate, maxYear);
  const haystack = [
    candidate.title,
    candidate.fileTitle,
    candidate.creator,
  ...
  const excludedTerm = WEEKLY_ARCHIVE_EXCLUDE_TERMS.find((term) => haystack.includes(term));
  const requiredTerms = Array.isArray(block.requireAny) ? block.requireAny.map((term) => String(term).toLowerCase()) : [];
  const missingRequired = requiredTerms.length
    ? !requiredTerms.some((term) => haystack.includes(term))
    : false;
  const reasons = [];

  if (safetyBlocked) reasons.push("explicit/porn filter matched");
  if (excludedTerm) reasons.push(`excluded term: ${excludedTerm}`);
  if (!language.ok) reasons.push(language.reason);
  if (tooShort) reasons.push(`too short for block minimum (${Math.round(duration)}s < ${minDuration}s)`);
  if (tooNew) reasons.push(`newer than block cutoff (${maxYear})`);
  if (missingRequired) reasons.push("missing required block keywords");
  if (!reasons.length) {
    reasons.push(quality.label === "reject" ? "quality score is weak, but no hard policy rejected it" : "passes current policy checks");
  }

  return {
    accepted: !safetyBlocked && !excludedTerm && language.ok && !tooShort && !tooNew && !missingRequired,
    reasons,
    quality,
    safety: {
      ok: !safetyBlocked,
      explicit: safetyBlocked
    },
    language,
    duration: {
      seconds: duration,
    ...
    archive: {
      id: candidate.archiveId || "",
      file: candidate.archiveFile || "",
      title: candidate.title || candidate.fileTitle || "",
...
export async function auditInternetArchiveSearch(query, { rows = 8, block = {} } = {}) {
  const results = await searchInternetArchiveSources(query, rows, { includeUnsafe: true });
  return results.map((candidate) => ({
    ...candidate,
    decision: explainArchiveCandidate(candidate, block)
  }));
}
